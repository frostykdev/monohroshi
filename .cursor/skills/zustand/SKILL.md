...
const initialState = {
  // default values only (no actions)
};
...
**Rules:**
- Always define `initialState` as a separate `const` above `create`.
- Always use `immer` middleware.
- `devtools` middleware is optional — add it when you need Redux DevTools inspection.

## When to Use Immer vs Simple `set`

**Use Immer (`set((state) => { state.x = y })`):**

- Mutating nested objects or arrays (toggling items, pushing to arrays, updating nested fields)
- Any time the update reads and then modifies existing state in place

**Use Simple `set({ key: value })`:**

- Replacing a top-level field with a completely new value
- Toggling a boolean
- Setting a single primitive field

```ts
// Simple set — replacing a value
setSelectedCurrencyCode: (code) =>
  set({ selectedCurrencyCode: code }, false, "setSelectedCurrencyCode"),

// Immer — mutating an array in place
markStepComplete: (id) =>
  set((state) => {
    if (!state.completedSteps.includes(id)) {
      state.completedSteps.push(id);
    }
  }, false, "markStepComplete"),
```
